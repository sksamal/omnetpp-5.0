...
#include <FSO_CoreControlLogic.h>

Define_Module(FSO_CoreControlLogic);

FSO_CoreControlLogic::~FSO_CoreControlLogic(){
	   free(recvBurstCounter);
	   free(schedBurstCounter);
}

void FSO_CoreControlLogic::initialize(){
   dropCounter = 0;
   WATCH(dropCounter);
   ...
   coreInput = check_and_cast<FSO_CoreInput*>((parent->getParentModule())->getSubmodule("oxcInput"));
   coreOutput = check_and_cast<FSO_CoreOutput*>((parent->getParentModule())->getSubmodule("oxcOutput"));
...
void FSO_CoreControlLogic::handleMessage(cMessage *msg){
   ...
   cout<<"CoreControl:: Msg arrived, port="<<arrivalPort<<", burstColour="<<burstColour<<", destlabel="<<destLabel<<endl;

   OBS_CoreRoutingTableEntry *result = routingTable->getEntry(arrivalPort,burstColour,destLabel);
   if(result == NULL) throw cRuntimeError("Error in routing table query (Control Unit id: %d) for (arrival,color,label)=(%d,%d,%d)",getId(),arrivalPort,burstColour,destLabel);
   ...
   if(outColour == -9){ // * option. Choose the lambda with closest horizon
	...
	if(lambda == -1){ // if none of the lambdas is available
        	delete msg;
        	dropCounter++;
        	cout<<"CoreControl:: No lambdas available, bcp(outPort="<<outPort<<",outColor="<<outColour<<",outLabel="<<outLabel<<"dropped"<<endl;
	    ...
	    //Check if channel is free at the burst arrival moment. (outColour is actually a colour, that's why I convert it using getLambdaByColour method)
      	...
      	if(gatesHorizon->getHorizon(outPort,lambda) > burstArrival ){
         	...
         	delete msg;
         	dropCounter++;
            cout<<"CoreControl:: Channel with lambda="<<lambda<<"unavailable, bcp dropped"<<endl;
   ...
   cout<<"CoreControl:: Scheduling msg to connect inGate="<<inGate<< "(arrivalPort="<<arrivalPort<<" and outGate="<<coreOutput->getOXCGate(outPort,lambda)<< "(outport="<<outPort<<",lambda="<<lambda<<")"<<endl;
   ...
   cout<<"CoreControl:: color of (port="<<outPort<<",lambda="<<lambda<<") is "<<coreOutput->getColourByLambda(outPort,lambda)<<endl;
   ...
   cout<<"CoreControl:: BCP sent outPort="<<outPort<<", outColour="<<bcp->getBurstColour()<<", outLabel="<<bcp->getLabel()<<endl;
...
void FSO_CoreControlLogic::finish(){
